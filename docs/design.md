...
The detailed reasoning behind [the README](../README.md): every design
decision with its known limits, the repository layout, a full sample run, the
spreadsheet defects the parser handles, the boundary to the iPaaS with the
event contract, and the Workato recipe.
...
tests/               201 tests covering normalization, matching, diffing,
...
| | |
|---|---|
| ![Recipe canvas](screenshots/02-recipe-canvas.png) | ![Job history](screenshots/03-job-history.png) |
...
| ![Duplicate stopped](screenshots/05-dedup-stopped-job.png) | ![Error monitor](screenshots/07-error-monitor.png) |
...
Also: [the recipe in its project](screenshots/01-project-recipe.png),
[a full run with the rendered notification](screenshots/04-successful-job.png),
and the lookup tables:
[orientation sessions](screenshots/08-orientation-sessions.png),
[role requirements](screenshots/09-role-requirements.png),
[processed events ledger](screenshots/06-processed-events.png).
